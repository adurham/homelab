...
"""
Cleanup Orphaned Automations Script
...
def main():
    ...
    print("🧹 CLEANUP ORPHANED AUTOMATIONS")
    print("=" * 50)
    ...
    for automation in orphaned:
        ...
        alias = automation.get('alias', 'No alias')
        enabled = automation.get('enabled', True)
        status = "Enabled" if enabled else "Disabled"
        print(f"  • {automation_id} ({alias}) - {status}")
    
    print("")
    
    # Confirm deletion
    response = input("🗑️  Do you want to delete these orphaned automations? (y/N): ")
    if response.lower() not in ['y', 'yes']:
        print("❌ Cleanup cancelled")
        return
    
    print("\n🗑️  Deleting orphaned automations...")
