#!/usr/bin/env python3
"""
Extract Home Assistant entities with API token
"""
...
def extract_entities_with_token(token):
    """Extract entities using API token"""
    ...
    try:
        ...
        if e.code == 401:
            print("💡 Invalid or expired token. Please check your API token.")
        return False
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python3 get_entities_with_token.py <API_TOKEN>")
        print("\nTo get an API token:")
        print("1. Go to http://192.168.86.2:8123/profile")
        print("2. Scroll down to 'Long-Lived Access Tokens'")
        print("3. Click 'Create Token'")
        print("4. Give it a name and copy the token")
        sys.exit(1)
    
    token = sys.argv[1]
    success = extract_entities_with_token(token)
